...
# Script to try to reproduce the problem with inconsistent steady state sizes
# and flows for some compartments in a network, see the scripts shared by Matt
# Dunkle in April 2024 for the original issue with Trinidad streams data.

# Notes about "no bug"/"bug" in section headings is based on isotracer code in commit 78b19fe
# Bugs seemed to be fixed in isotracer commit cadc6d7
...
### * Closed network, single unit, no steady, no split (no bug)
...
params <- summary(r)$statistics[,1]
params <- c(params, lambda_NH4 = 0, lambda_algae = 0, lambda_daphnia = 0)
mp <- set_params(m, params)
pp <- project(mp, end = 5)
plot(pp)
...
for (i in sample(seq_len(nrow(zf)), 20)) {
  x <- check_row_i(i)
  print(all(x[["from"]] - x[["to"]] < 1e-13))
}

### * Closed network, multiple units, no steady, no split (no bug)
...
r <- run_mcmc(m, chains = 2, iter = 500)
traceplot(r)

params <- summary(r)$statistics[,1]
for (g in c("LL|gup", "UL|gup", "LL|riv", "UL|riv")) {
  for (c in c("NH4", "algae", "daphnia")) {
    p <- setNames(0, paste0("lambda_", c, "|", g))
    params <- c(params, p)
  }
}
mp <- set_params(m, params)
pp <- project(mp, end = 5)
plot(pp, facet_row = "group")
...
### * Open network, multiple units, one steady, no split (no bug)
...
for (g in c("LL|gup", "UL|gup", "LL|riv", "UL|riv")) {
  for (c in c("NH4")) {
    p <- setNames(1, paste0("lambda_", c, "|", g))
    params <- c(params, p)
  }
}
mp <- set_params(m, params)
...
### * Open network, multiple units, one steady, one split (bug)
...
### * Open network, single unit, one steady, one split (bug)
