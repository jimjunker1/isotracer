...
plot_nm_row_timeline <- function(nm_row_timeline) {
    ...
    axis(1)
    lines(c(0, 0), c(0, 1), col = "lightgray")
    lines(rep(z[["maxTime"]], 2), c(0, 1), col = "lightgray")
    for (i in seq_along(z[["eventTimes"]])) {
        lines(rep(z[["eventTimes"]], 2), c(0, 1), col = "red", lty = 2, lwd = 2)
    }
    points(z[["obsTimes"]], y = rep(0.5, length(z[["obsTimes"]])),
